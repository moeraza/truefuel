...
import { Leaf, Droplets, DollarSign, FlaskConical, ArrowRight, Minus, ShoppingCart } from "lucide-react";
...
function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[hsl(160,40%,14%)] to-[hsl(160,30%,20%)] text-white">
      <div className="absolute inset-0 opacity-10">
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="topo" x="0" y="0" width="200" height="200" patternUnits="userSpaceOnUse">
              <path d="M 0 80 Q 50 60 100 80 T 200 80" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.4"/>
              <path d="M 0 120 Q 50 100 100 120 T 200 120" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.3"/>
              <path d="M 0 40 Q 50 20 100 40 T 200 40" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.2"/>
              <path d="M 0 160 Q 50 140 100 160 T 200 160" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.2"/>
            </pattern>
          </defs>
      ...
      <div className="relative max-w-4xl mx-auto px-6 py-20 md:py-28">
        <Badge variant="secondary" className="mb-6 bg-white/15 text-white/90 border-white/10" data-testid="badge-early-bird">
          Early Bird Pricing — Limited Time
        </Badge>
        <h1 className="font-display text-3xl md:text-4xl lg:text-[2.75rem] font-bold leading-[1.1] tracking-tight mb-5" data-testid="text-hero-title">
          Fuel your run for pennies,<br className="hidden sm:block" />
          not dollars.
        </h1>
        <p className="text-lg text-white/75 max-w-xl mb-8 leading-relaxed">
          ...
          Cut your fueling costs by 80% and keep hundreds of single-use packets out of landfills.
        </p>
        <div className="flex flex-wrap gap-3">
          <Link href="/shop">
            <Button size="lg" className="bg-white text-[hsl(160,50%,22%)] font-semibold" data-testid="button-shop-now">
              Shop now
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </Link>
          <Link href="/how-to">
            <Button size="lg" variant="outline" className="border-white/30 text-white bg-white/5" data-testid="button-how-it-works">
              How it works
            </Button>
          </Link>
        </div>
    ...
    <section className="max-w-4xl mx-auto px-6 py-16" id="products">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {products?.map((product) => (
          <Card key={product.id} className="p-5 flex flex-col gap-4" data-testid={`card-product-${product.id}`}>
...
function MissionSection() {
  return (
    <section className="border-t border-border">
      <div className="max-w-4xl mx-auto px-6 py-16">
        <h2 className="font-display text-xl font-bold mb-4">Why we built TrueFuel</h2>
